namespace Recognizer
{
	public static class GrayscaleTask
	{
		...
		{
			var row = original.GetUpperBound(0) + 1;
			var columns = original.Length / row;

			var grayscale = new double[row, columns];
			for (var i = 0; i < row; i++)
				for (var j = 0; j < columns; j++)
					grayscale[i, j] = (0.299 * original[i, j].R + 0.587 * original[i, j].G
